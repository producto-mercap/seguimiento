...
- `mantenimiento`: Datos editables de mantenimiento
  - Campos: `id_proyecto` (FK), `estado`, `fecha_inicio`, `fecha_fin`, `observaciones`, `updated_at`
- `proyectos_externos`: Datos editables de proyectos (externos e internos)
  - Campos: `id_proyecto` (FK), `estado`, `fecha_inicio`, `fecha_fin`, `observaciones`, `accionables` (legacy), `fecha_accionable` (legacy), `asignado_accionable` (legacy), `updated_at`
- `accionables_proyecto`: Accionables múltiples por proyecto
  - Campos: `id`, `id_proyecto` (FK), `fecha_accionable`, `asignado_accionable`, `accionable`, `estado`, `created_at`, `updated_at`
- `epics_proyecto`: Epics sincronizados de Redmine
  - Campos: `id_proyecto` (FK), `epic_id` (ID en Redmine), `subject`, `status`, `total_estimated_hours`, `total_spent_hours`, `proyecto_padre`, `nombre_proyecto_padre`, `cf_23`, `cf_21`, `cf_22`, `cf_15`, `created_at`, `updated_at`
  - Clave única: `(id_proyecto, epic_id)`
- `subproyectos`: Subproyectos (proyectos con `linea_servicio = 'Hereda'`)
  - Campos: `id_subproyecto`, `id_proyecto` (FK), `proyecto_padre`, `nombre`, `estado`, `overall`, `alcance`, `costo`, `plazos`, `riesgos`, `avance`, `created_at`, `updated_at`
  - Clave única: `(id_proyecto, proyecto_padre)`
- `productos_equipos`: Configuración de productos y equipos
  - Campos: `id`, `producto`, `equipo`, `id_equipo_redmine` (cf_75), `producto_redmine` (cf_19, opcional), `codigo_proyecto_padre` (opcional)
- `pedidos_equipos`: Pedidos entre equipos (módulo Sync)
  - Campos: `id`, `equipo_solicitante`, `equipo_responsable`, `descripcion`, `fecha_planificada_entrega`, `estado` (Pendiente, En curso, Bloqueado, Realizado), `comentario`, `created_at`, `updated_at`
...
- Retorna: Métricas del dashboard por producto
  - `producto`: Nombre del producto
  - `total_equipos`: Cantidad de equipos únicos
  - `total_clientes`: Cantidad de clientes únicos
  - `proyectos_en_curso`: Cantidad de proyectos con estado "En curso", "Testing", "Entregado" o "Rework"
...
LOGIN_PASSWORD_ADMIN=password_for_admin (opcional)
...
### MantenimientoModel
- `obtenerTodos(filtros)`: Obtiene mantenimientos desde `v_mantenimiento_completo` con filtros (producto, equipo, cliente, búsqueda)
- `obtenerPorId(id_proyecto)`: Obtiene un mantenimiento específico
- `actualizar(id_proyecto, datos)`: Actualiza campos editables (estado, fechas, observaciones)

### ProyectosExternosModel
- `obtenerTodos(filtros)`: Obtiene proyectos desde `v_proyectos_externos_completo` con filtros avanzados
- `obtenerPorId(id_proyecto)`: Obtiene un proyecto específico
- `obtenerCategoriasEquipo(equipo)`: Obtiene categorías disponibles para un equipo
- `obtenerSubproyectos(ids_proyectos)`: Obtiene subproyectos para múltiples proyectos (optimizado)
- `obtenerSubproyectosPorPadre(filtros)`: Obtiene subproyectos filtrados por proyecto padre
- `actualizar(id_proyecto, datos, actualizarTimestamp)`: Actualiza campos editables
- `obtenerMetricasDashboard()`: Obtiene métricas por producto

### EpicsProyectoModel
- `obtenerPorProyecto(id_proyecto)`: Obtiene todos los epics de un proyecto
- `obtenerTotalesPorProyecto(id_proyecto)`: Calcula totales (horas, fechas) de epics
- `guardarEpics(id_proyecto, epics)`: Guarda/actualiza múltiples epics (ON CONFLICT)
- `obtenerEpicsSecundariosPorProyectos(ids_proyectos)`: Detecta epics con proyecto_padre diferente
- `tieneEpics(id_proyecto)`: Verifica si un proyecto tiene epics

### SubproyectosModel
- `obtenerPorProyecto(id_proyecto)`: Obtiene subproyectos de un proyecto
- `obtenerPorProyectos(ids_proyectos)`: Obtiene subproyectos para múltiples proyectos (optimizado)
- `crearOActualizar(id_proyecto, proyecto_padre, nombre)`: Crea o actualiza un subproyecto
- `actualizar(id_subproyecto, datos)`: Actualiza campos editables de un subproyecto

### AccionablesProyectoModel
- `obtenerPorProyecto(id_proyecto)`: Obtiene todos los accionables de un proyecto
- `crear(id_proyecto, datos)`: Crea un nuevo accionable
- `actualizar(id, datos)`: Actualiza un accionable
- `eliminar(id)`: Elimina un accionable

### ProductosEquiposModel
- `obtenerTodos()`: Obtiene productos con sus equipos agrupados
- `obtenerEquiposPorProducto(producto)`: Obtiene equipos de un producto
- `obtenerProductos()`: Obtiene lista de productos únicos
- `obtenerEquipos()`: Obtiene lista de equipos únicos
- `obtenerProductoRedmine(producto)`: Obtiene el nombre del producto en Redmine
- `crear(datos)`: Crea configuración de producto/equipo
- `actualizar(id, datos)`: Actualiza configuración
- `eliminar(id)`: Elimina configuración

### PedidosEquiposModel
- `obtenerTodos(filtros)`: Obtiene pedidos con filtros (equipos, estados, fechas, búsqueda)
- `obtenerPorId(id)`: Obtiene un pedido específico
- `obtenerEquipos()`: Obtiene lista de equipos únicos (desde pedidos)
- `crear(datos)`: Crea un nuevo pedido
- `actualizar(id, datos)`: Actualiza un pedido
- `eliminar(id)`: Elimina un pedido
...
| `subject` | Redmine | ❌ |
| `status` | Redmine | ❌ |
| `total_estimated_hours` | Redmine | ❌ |
| `total_spent_hours` | Redmine | ❌ |
| `proyecto_padre` | Redmine | ❌ |
| `nombre_proyecto_padre` | Redmine | ❌ |
| `cf_23` | Redmine | ❌ |
| `cf_21` | Redmine (fecha planificada inicio) | ❌ |
| `cf_22` | Redmine (fecha planificada fin) | ❌ |
| `cf_15` | Redmine (fecha real finalización) | ❌ |
...
|-------|--------|----------|
| `id_proyecto` | Local | ❌ |
| `fecha_accionable` | Local | ✅ |
| `asignado_accionable` | Local | ✅ |
| `accionable` | Local | ✅ |
| `estado` | Local | ✅ |
...
8. **Autenticación**: El sistema usa JWT almacenado en cookies. Hay dos niveles de acceso:
   - Usuario normal: `LOGIN_PASSWORD`
   - Administrador: `LOGIN_PASSWORD_ADMIN` (opcional, si no se configura, solo existe usuario normal)
...
11. **Subproyectos**: Los subproyectos se identifican por tener `linea_servicio = 'Hereda'` y un `proyecto_padre` que existe en la BD. Se almacenan en la tabla `subproyectos` con campos adicionales de evaluación (`overall`, `alcance`, `costo`, `plazos`, `riesgos`, `avance`).

12. **Epics Secundarios**: Los epics pueden tener un `proyecto_padre` diferente al `id_proyecto` al que pertenecen. Esto permite identificar subproyectos relacionados. La función `obtenerEpicsSecundariosPorProyectos()` detecta estos casos.

13. **Tabla de Accionables**: El nombre real de la tabla en la BD es `accionables_proyectos` (plural), aunque el modelo se llama `AccionablesProyectoModel` (singular).

14. **Timezone**: La base de datos está configurada para usar `America/Argentina/Buenos_Aires` en todas las conexiones.
